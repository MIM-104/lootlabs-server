...
async function createDailyLinks() {
  try {
    ...
    pasteParams.append("api_paste_expire_date", "1D");
    ...
    console.error("Daily link generation failed:", error.response?.data || error.message);
    throw error;
  }
}
...
app.post("/generate-links", async (req, res) => {
  try {
    ...
    if (!currentLinks || Date.now() > keyExpiration) {
      currentLinks = await createDailyLinks();
      keyExpiration = Date.now() + ONE_DAY;
      console.log("Generated new daily links:", currentLinks);
    }
...
app.listen(3000, () => console.log("Key server running on port 3000"));
